...
- Fixed `http.response.status_code`, `error.type`, and span error status now always captured in `LLMTracingAdapter.registerResponse()` even when the response body is empty or non-parseable JSON (previously these were silently dropped for malformed/empty error bodies).
