...
- Fixed `LLMTracingAdapter.registerRequest` to set cross-cutting attributes (`gen_ai.api_base`, `gen_ai.system`, `gen_ai.provider.name`, `server.address`, `server.port`) before calling handler parsing, ensuring they are always present on the span even when the handler throws an exception
