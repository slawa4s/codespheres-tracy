...
- Added `FilesOpenAIApiEndpointHandler` covering all five OpenAI Files API routes (`files.create`, `files.list`, `files.retrieve`, `files.delete`, `files.content.retrieve`) with span attributes `openai.api.type`, `gen_ai.operation.name`, `tracy.request.purpose`, `tracy.request.file.size_bytes`, and `tracy.response.file.id`
