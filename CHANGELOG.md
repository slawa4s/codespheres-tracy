...
- Added `error.type` attribute (set to the HTTP status code string, e.g. `"400"`) on all error spans in `LLMTracingAdapter.registerResponse`, satisfying the OTel semantic-conventions `error.type` registry attribute requirement.
