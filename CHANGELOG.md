...
- Renamed non-registry `gen_ai.*` batch attributes in `AnthropicBatchesEndpointHandler` to the `anthropic.*` namespace: `gen_ai.request.batch.size` → `anthropic.batch.request_size`; `gen_ai.response.batch.id/processing_status/created_at/expires_at` → `anthropic.batch.id/processing_status/created_at/expires_at`; `gen_ai.response.batch.request_counts.*` → `anthropic.batch.request_counts.*`
