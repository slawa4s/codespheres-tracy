...
- Fixed `OpenTelemetryOkHttpInterceptor` binary-response fallback: the `else` branch now emits a `buildJsonObject` containing `_tracy_binary_size_bytes` (from `Content-Length`) instead of an always-empty `JsonObject`, enabling downstream handlers (e.g. `AudioSpeechOpenAIApiEndpointHandler`) to set `tracy.response.audio.size_bytes` for binary responses such as `POST /v1/audio/speech`
