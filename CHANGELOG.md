...
- Fixed OpenAI Responses API: `tracy.response.object` is now emitted for every response (previously the `object` field was listed as mapped but never extracted, causing it to be silently dropped).
