...
- Fixed `http.response.status_code` (renamed from non-standard `http.status_code`) being unset when `getResponseBodyAttributes` throws: the attribute is now written before the body-parse guard in `LLMTracingAdapter.registerResponse`, ensuring it is always emitted regardless of response body shape.
