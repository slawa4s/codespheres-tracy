...
- Fixed potential batch payload leak in `AnthropicLLMTracingAdapter`: added `"requests"` to `mappedRequestAttributes` so `populateUnmappedAttributes` no longer emits `tracy.request.requests` containing the full serialised batch body
