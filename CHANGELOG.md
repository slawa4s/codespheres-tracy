...
- Added `BatchesOpenAIApiEndpointHandler` covering four OpenAI Batch API routes (`batches.create`, `batches.list`, `batches.retrieve`, `batches.cancel`) with `openai.api.type = "batches"`, `gen_ai.operation.name`, request attributes `tracy.request.batch.{endpoint,completion_window,output_expires_after.anchor,output_expires_after.seconds,input_file.id}` and `tracy.request.metadata.keys`, and response attributes `tracy.batch.{id,status,created_at,request_counts.total,request_counts.completed,request_counts.failed}`
