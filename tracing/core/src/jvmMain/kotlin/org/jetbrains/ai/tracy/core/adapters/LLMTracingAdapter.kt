...
abstract class LLMTracingAdapter(private val genAISystem: String) {
    fun registerRequest(span: Span, request: TracyHttpRequest): Unit = runCatching {
        ...
        // Set core identification attributes before provider-specific parsing so they
        // are always recorded even if getRequestBodyAttributes throws an exception.
        ...
        fun Span.populateUnmappedAttributes(
            ...
            body.entries.forEach { (key, value) ->
                if (key !in (mappedAttributes)) {
                    ...
                    setAttribute(attributeKey, if (value is JsonPrimitive) value.content else value.toString())
                }
            }
        }
