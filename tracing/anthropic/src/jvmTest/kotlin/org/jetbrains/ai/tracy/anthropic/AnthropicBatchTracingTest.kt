...
class AnthropicBatchTracingTest : BaseAITracingTest() {
     ...
     * - `error.type` set to the exception class name
     * - `server.address` and `server.port` set from the client's base URL
     */
    @Test
     ...
     * the request is serialised and dispatched (e.g., when `BatchCreateParams.builder().build()`
     * is called without setting any requests, causing the SDK to throw at build-time).
     *
     * The replacement is performed via reflection on the `batches$delegate` lazy field inside
     * the [com.anthropic.services.blocking.MessageServiceImpl] so that [instrument] can
     * subsequently wrap it with the tracing proxy.
     */
    private fun injectThrowingBatchService(client: AnthropicClient) {
        ...
        // Swap the cached value inside the existing Lazy rather than replacing it.
        // `batches$delegate` is `private final kotlin.Lazy`; writing to a final field via
        // reflection is silently ignored on JVM 17+. Instead, we force the Lazy to initialise
        // (so `_value` holds a real object), then overwrite `_value` — a `@Volatile private var`
        // in SynchronizedLazyImpl — with our throwing proxy.
        ...
        while (cls != null) {
            try {
                ...
                // Ensure the Lazy is initialised before we reach into _value.
