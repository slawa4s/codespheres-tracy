...
class AnthropicBatchesTracingTest : BaseAITracingTest() {
    ...
    fun `test batch create sets operation name and batch attributes`() = runTest {
        withMockServer { server ->
            ...
            assertEquals(1L, trace.attributes[AttributeKey.longKey("gen_ai.batch.request_count")])
            assertEquals("claude-haiku-4-5-20251001", trace.attributes[AttributeKey.stringKey("gen_ai.request.model")])
            ...
            assertEquals(1L, trace.attributes[AttributeKey.longKey("gen_ai.batch.request_counts.processing")])
        }
    }
    ...
    @Test
    fun `test batch list sets operation name and list attributes`() = runTest {
        withMockServer { server ->
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
            ...
            assertEquals(2L, trace.attributes[AttributeKey.longKey("gen_ai.batch.list.count")])
            assertEquals(false, trace.attributes[AttributeKey.booleanKey("gen_ai.batch.list.has_more")])
        }
    }
    ...
    fun `test batch cancel sets operation name and batch attributes`() = runTest {
        withMockServer { server ->
            ...
            assertEquals(BATCH_ID, trace.attributes[AttributeKey.stringKey("gen_ai.response.id")])
            assertEquals("canceling", trace.attributes[AttributeKey.stringKey("gen_ai.batch.processing_status")])
        }
    }
