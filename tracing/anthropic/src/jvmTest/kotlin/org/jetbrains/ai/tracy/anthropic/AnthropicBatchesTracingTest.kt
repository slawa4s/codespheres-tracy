...
class AnthropicBatchesTracingTest : BaseAITracingTest() {
    ...
    @Test
    fun batchCreateSetsGenAiRequestBatchSize() = runTest {
        withMockServer { server ->
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
                    .setHeader("Content-Type", "application/json")
                    .setBody("""{"id":"msgbatch_01","type":"message_batch","processing_status":"in_progress","request_counts":{"processing":3,"succeeded":0,"errored":0,"canceled":0,"expired":0},"ended_at":null,"created_at":"2025-01-01T00:00:00Z","expires_at":"2025-01-02T00:00:00Z","archived_at":null,"cancel_initiated_at":null,"results_url":null}""")
            )
            ...
            val body = """
                {
                  "requests": [
                    {"custom_id":"r1","params":{"model":"claude-3-5-haiku-20241022","max_tokens":10,"messages":[{"role":"user","content":"A"}]}},
                    {"custom_id":"r2","params":{"model":"claude-3-5-haiku-20241022","max_tokens":10,"messages":[{"role":"user","content":"B"}]}},
                    {"custom_id":"r3","params":{"model":"claude-3-5-haiku-20241022","max_tokens":10,"messages":[{"role":"user","content":"C"}]}}
                  ]
                }
            """.trimIndent().toRequestBody(JSON)
            ...
            assertEquals(
                3L,
                trace.attributes[AttributeKey.longKey("anthropic.batch.request_size")],
                "anthropic.batch.request_size should also equal the number of entries in the requests array"
            )
        }
    }
