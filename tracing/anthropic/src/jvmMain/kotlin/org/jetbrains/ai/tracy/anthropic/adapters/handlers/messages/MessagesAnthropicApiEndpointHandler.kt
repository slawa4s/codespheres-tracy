...
package org.jetbrains.ai.tracy.anthropic.adapters.handlers.messages
 ...
 */
internal class MessagesAnthropicApiEndpointHandler(
    ...
    private fun createSpanName(span: Span, body: JsonObject): String? {
        val readable = span as? ReadableSpan
        val operationName = readable?.attributes?.get(GEN_AI_OPERATION_NAME)
        val model = readable?.attributes?.get(GEN_AI_REQUEST_MODEL)
        ...
        return if (operationName != null && model != null) {
            "$operationName $model"
        } else {
            null
        }
