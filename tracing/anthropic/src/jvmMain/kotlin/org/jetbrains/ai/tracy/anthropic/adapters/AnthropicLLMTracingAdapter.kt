...
class AnthropicLLMTracingAdapter : LLMTracingAdapter(genAISystem = GenAiSystemIncubatingValues.ANTHROPIC) {
    ...
    override fun getRequestBodyAttributes(span: Span, request: TracyHttpRequest) {
        if (isBatchesRequest(request.url)) {
    ...
    override fun getResponseBodyAttributes(span: Span, response: TracyHttpResponse) {
        if (isBatchesRequest(response.url)) {
    ...
    /**
     * Returns true when the URL targets the Anthropic Message Batches API
     * (i.e., the path contains "batches" after "messages").
     */
    private fun isBatchesRequest(url: TracyHttpUrl): Boolean =
        url.pathSegments.contains("batches")
