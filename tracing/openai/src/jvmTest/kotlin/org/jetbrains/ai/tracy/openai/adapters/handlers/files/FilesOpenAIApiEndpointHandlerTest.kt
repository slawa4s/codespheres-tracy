...
class FilesOpenAIApiEndpointHandlerTest : BaseOpenAITracingTest() {
    ...
    @Test
    fun `test LIST files endpoint traces pagination query parameters`() = runTest(timeout = 3.minutes) {
        withMockServer { server ->
            val client = createOpenAIClient(
                url = server.url("/").toString(),
                apiKey = MOCK_API_KEY,
            ...
            server.enqueue(enqueueFileListResponse(count = 1))

            val limit = 5L
            val after = "file-cursor-abc"
