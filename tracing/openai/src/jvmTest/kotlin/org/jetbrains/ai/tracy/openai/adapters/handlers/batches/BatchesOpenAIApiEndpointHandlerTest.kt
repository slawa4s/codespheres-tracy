...
class BatchesOpenAIApiEndpointHandlerTest : BaseOpenAITracingTest() {
    ...
    @Test
    fun `test LIST batches endpoint traces pagination query parameters`() = runTest(timeout = 3.minutes) {
        withMockServer { server ->
            val client = createOpenAIClient(
                url = server.url("/").toString(),
                apiKey = MOCK_API_KEY,
            ...
            server.enqueue(enqueueBatchListResponse(count = 1))

            val limit = 10L
            val after = "batch-cursor-abc"
