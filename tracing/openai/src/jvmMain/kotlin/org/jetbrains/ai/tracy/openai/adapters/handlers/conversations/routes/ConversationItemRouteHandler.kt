...
/**
 * Handles requests and responses for different conversation items API routes of OpenAI.
 */
internal interface ConversationItemRouteHandler {
    fun handleRequest(span: Span, request: TracyHttpRequest)
    fun handleResponse(span: Span, response: TracyHttpResponse)
}

/**
 * Extracts `conversation_id` from a path like `/v1/conversations/{conversation_id}/items`.
 */
internal fun extractConversationIdFromPath(url: TracyHttpUrl): String? {
    ...
    return if (conversationsIndex != -1 && segments.size > conversationsIndex + 1) {
        val potential = segments[conversationsIndex + 1]
