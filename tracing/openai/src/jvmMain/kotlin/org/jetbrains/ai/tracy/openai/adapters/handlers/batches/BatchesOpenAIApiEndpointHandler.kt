...
internal class BatchesOpenAIApiEndpointHandler : EndpointApiHandler {
    ...
    /** Batch API does not use SSE streaming. */
    override fun handleStreaming(span: Span, events: String) = Unit

    /**
     * Infers the batch operation from path segments and HTTP method.
     *
     * Path layout relative to the "batches" segment:
     * - no suffix + POST  → `batches.create`
     * - no suffix + GET   → `batches.list`
     * - `/{batch_id}` + GET → `batches.retrieve`
     * - `/{batch_id}/cancel` + POST → `batches.cancel`
     */
