 ...
 *
 * Sets `gen_ai.operation.name=conversations.items.retrieve`, `openai.api.type=conversations`,
 * and `gen_ai.conversation.id` from URL. On response reads `id`, `type`, `status`, and
 * `conversation_id` (falling back to URL).
 */
