...
internal class RetrieveConversationItemHandler : ConversationRouteHandler {
    override fun handleRequest(span: Span, request: TracyHttpRequest) {
        ...
        if (itemId != null) {
            span.setAttribute("gen_ai.request.conversation.item.id", itemId)
        } else {
    ...
    override fun handleResponse(span: Span, response: TracyHttpResponse) {
        ...
        body["id"]?.let { span.setAttribute("gen_ai.response.conversation.item.id", it.jsonPrimitive.content) }
        body["type"]?.let { span.setAttribute("gen_ai.response.conversation.item.type", it.jsonPrimitive.content) }
    }
}
