 ...
 *
 * The [GEN_AI_OPERATION_NAME] is set during request handling so that it is captured
 * even when the API returns an error response. Audio transcription/translation responses
 * do not contain an "object" field, so the [org.jetbrains.ai.tracy.openai.adapters.handlers.OpenAIApiUtils.setCommonResponseAttributes]
 * fallback cannot infer the operation name from the response body.
 *
        ...
        for (part in body.parts) {
            ...
            when (part.name) {
                ...
                null -> logger.warn { "Form data part with missing name ignored. Content type: '$contentType'" }
                    ...
                    // Other parts (language, prompt, timestamp_granularities, etc.) are not traced.
                }
            }
        }
    }
