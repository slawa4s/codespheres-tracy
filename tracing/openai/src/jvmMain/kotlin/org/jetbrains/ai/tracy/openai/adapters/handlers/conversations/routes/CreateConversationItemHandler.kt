...
/**
 * Handles the `POST /conversations/{id}/items` endpoint (route=ITEMS_CREATE).
 */
internal class CreateConversationItemHandler : ConversationRouteHandler {
    override fun handleRequest(span: Span, request: TracyHttpRequest) {
        ...
        body?.get("limit")?.jsonPrimitive?.content?.let { span.setAttribute("tracy.request.limit", it) }
        body?.get("order")?.jsonPrimitive?.content?.let { span.setAttribute("tracy.request.order", it) }
        body?.get("after")?.jsonPrimitive?.content?.let { span.setAttribute("tracy.request.after", it) }
    }
