 ...
 *
 * The Conversations API provides multiple endpoints for managing multi-turn conversations:
 * 1. `POST /conversations` - Create a new conversation
 * 2. `GET /conversations` - List conversations with pagination
 ...
 *
 * Each route handler sets gen_ai.operation.name explicitly to the correct dotted name
 * (e.g. "conversations.create") rather than inheriting the incorrect value from the
 * response body's "object" field (e.g. "conversation", "list", "conversation.deleted").
 * Conversation IDs are stored under gen_ai.conversation.id rather than gen_ai.response.id.
 *
