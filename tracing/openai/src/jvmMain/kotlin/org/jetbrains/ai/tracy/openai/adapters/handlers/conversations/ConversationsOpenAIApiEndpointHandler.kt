 ...
 * The `gen_ai.operation.name` attribute is always set from the route rather than from the
 * response body `object` field (which returns values like `"conversation"`, `"list"`, or
 * `"conversation.deleted"` that do not match the expected operation names).
 *
