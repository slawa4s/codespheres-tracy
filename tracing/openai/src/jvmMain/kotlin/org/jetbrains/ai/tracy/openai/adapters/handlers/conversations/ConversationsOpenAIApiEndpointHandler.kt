 ...
 *
 * This handler detects the specific route and traces accordingly, setting `gen_ai.operation.name`
 * from the detected route rather than from the response `object` field (which contains values like
 * "conversation", "list", or "conversation.deleted" that do not reflect the operation name).
 *
