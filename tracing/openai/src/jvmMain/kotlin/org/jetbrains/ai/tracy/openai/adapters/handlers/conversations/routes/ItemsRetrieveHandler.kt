...
/**
 * Handles the ITEMS_RETRIEVE endpoint: `GET /conversations/{conversation_id}/items/{item_id}`.
 *
 * Request attributes set:
 * - `gen_ai.conversation.id` from URL path
 *
 * Response attributes set:
 * - `tracy.conversation.item.id` from response body `id`
 * - `tracy.conversation.item.type` from response body `type`
 * - `tracy.conversation.item.status` from response body `status`
 */
internal class ItemsRetrieveHandler : ConversationRouteHandler {
