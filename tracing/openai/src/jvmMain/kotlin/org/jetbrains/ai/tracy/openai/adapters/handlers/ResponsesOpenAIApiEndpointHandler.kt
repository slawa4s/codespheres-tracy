        ...
        for (line in events.lineSequence()) {
            ...
            when (event["type"]?.jsonPrimitive?.content) {
                ...
                "response.completed", "response.done" -> {
                    ...
                    finishReason = when (val status = response["status"]?.jsonPrimitive?.content) {
                        "completed" -> "stop"
                        "failed" -> "error"
                        "cancelled" -> "cancelled"
                        "incomplete" -> response["incomplete_details"]?.jsonObject
                            ?.get("reason")?.jsonPrimitive?.content ?: "length"
                        else -> status
                    }
                }
            }
