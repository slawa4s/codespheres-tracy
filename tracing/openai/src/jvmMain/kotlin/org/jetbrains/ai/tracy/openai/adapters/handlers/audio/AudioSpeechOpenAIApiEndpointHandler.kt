 ...
 *
 * Extracts model, voice, response_format, and speed from the request. Reads the response
 * audio size from the reserved `_tracy.response.size_bytes` key injected by the interceptor,
 * and sets `gen_ai.response.audio.size_bytes` on the span.
 *
...
internal class AudioSpeechOpenAIApiEndpointHandler : EndpointApiHandler {
    ...
    override fun handleRequestAttributes(span: Span, request: TracyHttpRequest) {
        ...
        body["voice"]?.jsonPrimitive?.content?.let {
            span.setAttribute("gen_ai.request.audio.voice", it)
        }
