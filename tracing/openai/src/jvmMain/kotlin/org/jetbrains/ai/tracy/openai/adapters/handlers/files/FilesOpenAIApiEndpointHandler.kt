...
internal class FilesOpenAIApiEndpointHandler : EndpointApiHandler {
    ...
    /** Distinguishes between the five Files API routes. */
    ...
    /**
     * Infers the [FileRoute] from the URL path segments and HTTP method.
     *
     * Path layout relative to the "files" segment:
     * - no suffix → collection level (CREATE / LIST)
     * - `/{id}` → single file (RETRIEVE / DELETE)
     * - `/{id}/content` → file content (CONTENT_RETRIEVE)
     */
