...
/**
 * Handles `GET /conversations/{conversation_id}/items` — lists items with cursor-based pagination.
 *
 * Sets `gen_ai.operation.name=conversations.items.list`, `openai.api.type=conversations`,
 * and `gen_ai.conversation.id` from URL. Reads pagination query params (`limit`, `order`, `after`)
 * and on response reads list metadata (`data` count, `first_id`, `last_id`, `has_more`).
 */
