...
private class TracingPlugin(private val adapter: LLMTracingAdapter) {
    ...
    fun setup(config: HttpClientConfig<*>) {
        ...
        config.install(createClientPlugin("NetworkParamsPlugin") {
            ...
            transformResponseBody { response, content, typeInfo -> handleStreamingBody(response, content, typeInfo) }
        })
    }
    ...
    private suspend fun handleResponse(response: HttpResponse) {
        val ctx = response.spanContext() ?: return
        ...
        adapter.registerResponse(ctx.span, response = response.asResponseView(responseBody))
        ctx.span.end()
    }

    private fun handleStreamingBody(response: HttpResponse, content: ByteReadChannel, typeInfo: TypeInfo): Any? {
        val ctx = response.spanContext() ?: return null
        ...
        adapter.registerResponse(ctx.span, response = response.asResponseView(body))
        ...
        CoroutineScope(response.coroutineContext).launch(start = CoroutineStart.UNDISPATCHED) {
            try {
                ...
                ctx.span.setStatus(StatusCode.ERROR)
    ...
    private data class SpanContext(val isStreaming: Boolean, val span: Span)

    private fun HttpResponse.spanContext(): SpanContext? {
        val enabled = call.request.attributes[tracingEnabledKey]
        if (!enabled) return null
        val isStreaming = call.request.attributes.getOrNull(isStreamingRequestKey) ?: return null
        val span = call.request.attributes.getOrNull(httpSpanKey) ?: return null
        return SpanContext(isStreaming, span)
    }
